...
- REST `/v1/api/players`: 접속 감지용 `userId` 스냅샷과 Discord 탈출 목록용 게임 닉네임·`userId` 스냅샷
...
`rename(2)`하는 방식으로 원자적으로 교체합니다. 접속 감지용 스냅샷에는
`userId`와 서버 uptime만 기록합니다. 별도 탈출 목록 스냅샷에는 Discord의
`/pal escape` 선택 목록을 위해 게임 닉네임과 `userId`만 기록하며, 계정명, IP,
위치와 ping은 넣지 않습니다. REST API가 아직 준비되지 않았거나 일시적으로
실패하면 프로세스는 종료되지 않고 stderr에 경고를 남긴 뒤 다음 주기에 다시
시도합니다.
...
| `PALWORLD_OBSERVER_PLAYER_DIRECTORY_OUTPUT` | `/var/lib/palworld-observer/player-directory.snapshot` | 탈출 목록용 닉네임·`userId` 스냅샷 출력 경로 |
