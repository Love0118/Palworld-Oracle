...
플레이어가 접속하거나 퇴장하면 같은 채널에 현재 접속 인원과 REST가 제공하는
`userId`를 기록합니다. 서버 재시작으로 인한 전체 접속 해제는 개별 퇴장으로
기록하지 않습니다. `/pal escape` 선택 목록을 위해 observer는 온라인 플레이어의
게임 닉네임과 `userId`만 Discord 봇에 전달합니다. 계정명, IP, 위치와 ping은
observer 단계에서 버립니다. 관측 주기가 10초이므로 접속·퇴장 로그와 선택 목록은
최대 약 10초 늦을 수 있습니다.
